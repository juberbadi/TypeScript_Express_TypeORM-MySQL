...
const signupUser = async (req, res, next) => {
    ...
    try {
        ...
        let existingUser = await entityManager.findOne(User_1.User, { where: { email } });
        ...
        let record = await entityManager.insert(User_1.User, { name, email, password: hashedPassword });
...
const signinUser = async (req, res, next) => {
    ...
    try {
        const entityManager = (0, typeorm_1.getManager)();
        let user = await entityManager.findOne(User_1.User, { where: { email } });
...
const sendVerificationMail = async (req, res, next) => {
    ...
    try {
        ...
        await entityManager.update(User_1.User, { email: email }, { verifyToken: encryptedToken });
...
const verifyUserMail = async (req, res, next) => {
    ...
    try {
        ...
        const user = await entityManager.findOne(User_1.User, decodedToken.userId);
        // const user = await UserSchema.findByPk(decodedToken.userId)
        if (!user)
            return next((0, http_errors_1.default)(401, "Token Invalid"));
        await entityManager.update(User_1.User, user, { isUserVerified: true, verifyToken: "0" });
        // await user.update(
        //   { isUserVerified: true },
        //   { verifyToken: 0 }
        // );
...
const sendForgotPasswordMail = async (req, res, next) => {
    ...
    try {
        const entityManager = (0, typeorm_1.getManager)();
        const user = await entityManager.findOne(User_1.User, { where: { email } });
        // const user = await UserSchema.findOne({ where: { email } });
        if (!user)
            return next((0, http_errors_1.default)(404, "Email not valid!"));
        const encryptedToken = await bcrypt_1.default.hash(user.id.toString(), 8);
        ...
        // await user.update({ verifyToken: encryptedToken });
        await entityManager.update(User_1.User, user, { verifyToken: encryptedToken });
        res.status(200).json({ message: `Preview URL: %s, ${nodemailer_1.default.getTestMessageUrl(info)}` });
    }
    catch (error) {
        return next(http_errors_1.InternalServerError);
...
const verifyForgotMail = async (req, res, next) => {
    ...
    try {
        ...
        await entityManager.update(User_1.User, user, { password: encryptedPassword, verifyToken: "0" });
